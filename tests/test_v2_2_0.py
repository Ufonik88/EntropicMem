...
def test_upsert_triple_idempotent(engine):
    tid1 = engine.upsert_triple("Alice", "works_at", "Acme Corp")
    tid2 = engine.upsert_triple("Alice", "works_at", "Acme Corp")
    assert tid1 == tid2
    assert engine.triple_stats()["total"] == 1


def test_triple_neighbors(engine):
    engine.upsert_triple("Alice", "works_at", "Acme Corp")
    engine.upsert_triple("Acme Corp", "sells", "Security Systems")
    neigh = engine.triple_neighbors("Acme Corp")
    assert len(neigh) == 2
...
def test_find_entities_known():
    assert "Nous Research" in find_entities("Alice works at Nous Research")
    assert known_entity("alice")
    assert not known_entity("zorpblorp")


def test_extract_triples_relation_phrase():
    triples = extract_triples_from_text("Alice works at Nous Research")
    assert ("Alice", "works_at", "Nous Research") in triples
...
def test_extract_triples_from_engine(engine):
    engine.remember("Alice works at Nous Research and uses Google Drive",

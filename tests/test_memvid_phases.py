...
class TestGraphQuery:
    ...
    def test_extract_wikilinks(self):
        links = extract_wikilinks("See [[Budget]] and [[Event Plan|the event]]")
        assert "Budget" in links
        assert "Event Plan" in links
    ...
    def test_store_and_retrieve(self, db):
        store_links(db, "Finance/Budget.md", "Budget", [("Event", "for the event")])
        outgoing = get_outgoing_links(db, "Finance/Budget.md")
        assert len(outgoing) == 1
        assert outgoing[0]["target"] == "Event"

    def test_incoming_links(self, db):
        store_links(db, "Finance/Budget.md", "Budget", [("Event", "cost")])
        store_links(db, "Projects/Event.md", "Event", [("Budget", "see budget")])
        incoming = get_incoming_links(db, "Event")

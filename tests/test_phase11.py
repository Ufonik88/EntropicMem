...
class TestGraphAwareRecall:
    ...
    def test_expand_links_with_connections(self, engine):
        ...
        engine.remember("Budget allocation for event", title="Budget", domain="Finance")

        # Build link graph
        init_links_schema(engine.db)
        store_links(engine.db, "Projects/Event.md", "Event", [("Budget", "see budget")])
        engine.db.commit()

        results = engine.recall_hybrid("event", expand_links=True)
        assert len(results) >= 1

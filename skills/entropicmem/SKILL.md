...
`memory.provider: entropicmem`. Interactive: `memory` tool + `entropicmem_*`
tools. Cron: use the deterministic write helper (`entropicmem-cron-writes`
skill) — Hermes sets `skip_memory=True` in cron by design.

...
def _resolve_env() -> tuple[Path, Path]:
    ...
    index_db = Path(
        os.environ.get("ENTROPICMEM_INDEX_DB",
                        str(hermes_home_path() / "entropicmem" / "index.db"))
    )
...
def cmd_init(args) -> int:
    ...
    memory_path = hermes_home_path() / "entropicmem" / "memory.db"
...
def main() -> int:
    ...
    p_t_neighbors.add_argument("entity", help="Entity to query (e.g. 'Acme Corp')")
